...
class SpecializedNeuron(Module):
    ...
    Attributes:
        ...
        state (TensorLike): Current activation state
    ...
    def update(self,
              inputs: Union[List[float], TensorLike],
              dt: float = 0.1,
              tau_mod: float = 1.0,
    ...
    def get_weights(self) -> TensorLike:
        """
        Get the current weight vector.
        
        Returns:
            TensorLike: Copy of the weight vector
        """
        return tensor.copy(self.weights)
    
    def set_weights(self, weights: TensorLike) -> None:
        """
