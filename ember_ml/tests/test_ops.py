...
class TestTensorOps:
    ...
    def test_convert_to_tensor(self, setup_numpy_backend):
        ...
        assert x.shape == (3,)
        assert ops.all(x == tensor.convert_to_tensor([1, 2, 3]))
...
class TestMathOps:
    ...
    def test_add(self, setup_numpy_backend):
        ...
        z = m_ops.add(x, y)
        assert ops.all(z == tensor.convert_to_tensor([5, 7, 9]))
    ...
    def test_subtract(self, setup_numpy_backend):
        ...
        assert ops.all(z == tensor.convert_to_tensor([-3, -3, -3]))
    ...
    def test_multiply(self, setup_numpy_backend):
        ...
        assert ops.all(z == tensor.convert_to_tensor([4, 10, 18]))
    ...
    def test_divide(self, setup_numpy_backend):
        ...
        y = tensor.convert_to_tensor([4, 5, 6])
        z = m_ops.divide(x, y)
        assert ops.allclose(z, tensor.convert_to_tensor([0.25, 0.4, 0.5]))
    ...
    def test_exp(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([1, 2, 3])
        z = m_ops.exp(x)
        assert ops.allclose(z, np.exp(x))
    ...
    def test_log(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([1, 2, 3])
        z = m_ops.log(x)
        assert ops.allclose(z, np.log(x))

    def test_log10(self, setup_numpy_backend):
        """Test log10 function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([1, 10, 100])
        z = m_ops.log10(x)
        assert ops.allclose(z, np.log10(x))

    def test_log2(self, setup_numpy_backend):
        """Test log2 function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([1, 2, 4])
        z = m_ops.log2(x)
        assert ops.allclose(z, np.log2(x))

    def test_pow(self, setup_numpy_backend):
        """Test pow function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([1, 2, 3])
        y = tensor.convert_to_tensor([2, 3, 4])
        z = m_ops.pow(x, y)
        assert ops.allclose(z, np.power(x, y))

    def test_sqrt(self, setup_numpy_backend):
        """Test sqrt function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([1, 4, 9])
        z = m_ops.sqrt(x)
        assert ops.allclose(z, ops.sqrt(x))
    ...
    def test_square(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([1, 2, 3])
        z = m_ops.square(x)
        assert ops.allclose(z, np.square(x))
    ...
    def test_abs(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([-1, -2, -3])
        z = m_ops.abs(x)
        assert ops.allclose(z, np.abs(x))
    ...
    def test_negative(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([1, 2, 3])
        z = m_ops.negative(x)
        assert ops.allclose(z, np.negative(x))

    def test_sign(self, setup_numpy_backend):
        """Test sign function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([-1, 0, 1])
        z = m_ops.sign(x)
        assert ops.allclose(z, np.sign(x))
    ...
    def test_clip(self, setup_numpy_backend):
        ...
        z = m_ops.clip(x, 2, 4)
        assert ops.allclose(z, ops.clip(x, 2, 4))

    def test_sin(self, setup_numpy_backend):
        """Test sin function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([0, ops.pi / 2, ops.pi])
        z = m_ops.sin(x)
        assert ops.allclose(z, ops.sin(x))
    ...
    def test_cos(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([0, ops.pi / 2, ops.pi])
        z = m_ops.cos(x)
        assert ops.allclose(z, np.cos(x))

    def test_tan(self, setup_numpy_backend):
        """Test tan function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([0, ops.pi / 4, ops.pi / 2])
        z = m_ops.tan(x)
        assert ops.allclose(z, np.tan(x))
    ...
    def test_sinh(self, setup_numpy_backend):
        ...
        z = m_ops.sinh(x)
        assert ops.allclose(z, np.sinh(x))
    ...
    def test_cosh(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([0, 1, 2])
        z = m_ops.cosh(x)
        assert ops.allclose(z, np.cosh(x))
    ...
    def test_tanh(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([0, 1, 2])
        z = m_ops.tanh(x)
        assert ops.allclose(z, np.tanh(x))
    ...
    def test_sigmoid(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([0, 1, 2])
        z = m_ops.sigmoid(x)
        assert ops.allclose(z, 1 / (1 + np.exp(-x)))
    ...
    def test_softplus(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([0, 1, 2])
        z = m_ops.softplus(x)
        assert ops.allclose(z, np.log(1 + np.exp(x)))
    ...
    def test_relu(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([-1, 0, 1])
        z = m_ops.relu(x)
        assert ops.allclose(z, ops.stats.maximum(0, x))
    ...
    def test_softmax(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([1, 2, 3])
        z = m_ops.softmax(x)
        exp_x = np.exp(x - ops.stats.max(x))
        expected = exp_x / ops.stats.sum(exp_x)
        assert ops.allclose(z, expected)

    def test_gradient(self, setup_numpy_backend):
        """Test gradient function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([1, 2, 4, 7, 11])
        z = m_ops.gradient(x)
    ...
    def test_cumsum(self, setup_numpy_backend):
        ...
        x = tensor.convert_to_tensor([1, 2, 3])
        z = m_ops.cumsum(x)
        assert ops.allclose(z, np.cumsum(x))

    def test_eigh(self, setup_numpy_backend):
        """Test eigh function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([[1, 2], [2, 3]])
        w, v = m_ops.eigh(x)
        expected_w, expected_v = np.linalg.eigh(x)
        assert ops.allclose(w, expected_w)
        assert ops.allclose(v, expected_v)

    def test_mod(self, setup_numpy_backend):
        """Test mod function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([1, 2, 3])
        y = tensor.convert_to_tensor([2, 2, 2])
        z = m_ops.mod(x, y)
        assert ops.allclose(z, np.mod(x, y))
    ...
    def test_floor_divide(self, setup_numpy_backend):
        ...
        assert ops.allclose(z, np.floor_divide(x, y))

    def test_sort(self, setup_numpy_backend):
        """Test sort function."""
        m_ops = math_ops()
        x = tensor.convert_to_tensor([3, 1, 2])
        z = m_ops.sort(x)
        assert ops.allclose(z, np.sort(x))
...
class TestRandomOps:
    ...
    def test_random_uniform(self, setup_numpy_backend):
        ...
        assert x.shape == (2, 3)
        assert ops.all(x >= 0) and ops.all(x < 1)
