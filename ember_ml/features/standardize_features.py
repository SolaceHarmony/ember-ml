...
class Standardize:
    ...
    def fit(
        ...
        *,
        ...
        # Compute mean
        if with_mean:
            self.mean_ = stats.mean(X_tensor, axis=axis, keepdims=True)
        else:
            self.mean_ = None
        
        # Compute standard deviation
        if with_std:
            # Calculate variance directly using stats.var(ddof=0)
            # This might be more numerically stable than manual calculation.
            var = stats.var(X_tensor, axis=axis, keepdims=True, ddof=0)
