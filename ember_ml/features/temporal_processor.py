...
class TemporalStrideProcessor:
    ...
    def process_batch(self, data) -> Dict[int, TensorLike]:
        """
        ...
        """
        # Convert input to TensorLike if it's not already
    ...
    def _create_strided_sequences(self, data: tensor, stride: int) -> List[TensorLike]:
        """
        ...
        for i in range(num_windows):
            ...
            # Use data.numpy() only for slicing, then convert back to TensorLike
            # This is one of the few cases where numpy conversion is allowed
            data_np = data.numpy()
            window_np = data_np[start_idx:end_idx]
            window = tensor(window_np)
            windows.append(window)
            
        return windows
    
    def _apply_pca_blend(self, window_batch: List[TensorLike], stride: int) -> TensorLike:
        """
        Apply PCA-based temporal blending.
        
        Args:
            window_batch: Batch of windows (list of TensorLikes)
            stride: Stride length
        ...
        # Convert back to TensorLike
        return tensor(transformed_np)
    
    def get_explained_variance(self, stride: int) -> Optional[TensorLike]:
        """
        ...
        if stride in self.pca_models:
            return TensorLike(stats.sum(self.pca_models[stride].explained_variance_ratio_))
        return None
