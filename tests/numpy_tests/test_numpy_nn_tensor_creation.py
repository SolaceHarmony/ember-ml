...
def test_linspace():
    ...
    assert isinstance(result, tensor.EmberTensor)
    assert tensor.shape(result) == (5,)
    assert ops.allclose(tensor.to_numpy(result), tensor.linspacepace(0.0, 1.0, 5))

    result_int = tensor.linspace(0, 10, 3, dtype=tensor.int32)
    assert tensor.dtype(result_int) == tensor.int32
    assert tensor.convert_to_tensor_equal(tensor.to_numpy(result_int), tensor.linspacepace(0, 10, 3, dtype=np.int32))
