...
def test_tensor_cast_mlx(mlx_backend, dtype_pair): # Use fixture
    ...
    if 'float64' in [str(original_dtype), str(target_dtype)]:
        try:
            import mlx.core as mx
            temp_array = mx.array(1.0, dtype=mx.float32)
            _ = temp_array.astype(mx.float64)
...
def test_tensor_to_numpy_mlx(mlx_backend): # Use fixture
    ...
    assert isinstance(t_numpy, np.ndarray), "Not np.ndarray"
    assert np.allclose(t_numpy, np.array(data)), "Content mismatch"
