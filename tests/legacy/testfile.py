...
class StrideAwareWiredCfCCell(keras.layers.Layer):
    ...
    def __init__(
        ...
        self.input_mask = tf.constant(wiring.get_config()["sparsity_level"], dtype=tf.float32)
        self.recurrent_mask = tf.constant(np.random.binomial(1, wiring.get_config()["sparsity_level"], (self.units, self.units)), dtype=tf.float32)
        self.output_mask = tf.constant(np.random.binomial(1, wiring.get_config()["sparsity_level"], (self.units, self.units)), dtype=tf.float32)
