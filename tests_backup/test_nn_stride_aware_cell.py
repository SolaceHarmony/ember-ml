...
def wiring():
    ...
    return FullyConnectedMap(
        units=8,
        input_dim=8,  # Use a fixed input_dim
        output_dim=4  # Same as output_size in AutoNCP
    )  # Use FullyConnectedMap which is a genuine NeuronMap subclass
...
def test_stride_aware_cell_creation(backend_name, original_backend, wiring):
    ...
    try:
        ...
        # Create a cell
        cell = StrideAwareWiredCfCCell(neuron_map=wiring, stride_length=2, time_scale_factor=1.0)
        
        # Verify the cell properties
        # Use neuron_map.units for both sides
        assert cell.neuron_map.units == wiring.units
...
def test_stride_aware_cfc_creation(backend_name, original_backend, wiring):
    ...
    try:
        ...
        # Create a layer
        rnn_layer = StrideAwareCfC(neuron_map_or_cell=cell, return_sequences=True)
...
def test_stride_aware_cfc_mixed_memory(backend_name, original_backend, wiring):
    ...
    try:
        ...
        rnn_layer = StrideAwareCfC(neuron_map_or_cell=cell, return_sequences=True, mixed_memory=True)
...
def test_stride_aware_cell_forward(backend_name, original_backend, wiring):
    ...
    try:
        ...
        # Create input data
        batch_size = 2
        # Use a fixed input_dim since AutoNCP doesn't expose input_dim
        input_dim = 8
        ...
        # Verify output shape
        assert output.shape == (batch_size, cell.neuron_map.units)
    except ImportError:
        pytest.skip(f"{backend_name} backend not available")
...
def test_stride_aware_cfc_forward(backend_name, original_backend, wiring):
    ...
    try:
        ...
        # Verify output shape
        assert output.shape == (batch_size, seq_length, cell.neuron_map.units)
    except ImportError:
        pytest.skip(f"{backend_name} backend not available")
...
def test_serialization(backend_name, original_backend, wiring):
    ...
    try:
        ...
        assert new_cell.neuron_map.units == cell.neuron_map.units
